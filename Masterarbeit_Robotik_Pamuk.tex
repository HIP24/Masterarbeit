...
Isolating CPUs inolves removing all user-space threads and unbound kernel threads since bound kernel threads are tied to specific CPUs and hence cannot be moved. Also, modifying the \texttt{proc/irq/IRQ\_NUMBER/smp\_affinity} property of each Interrupt IRQ\_NUMBER in the system is part of this process.

Output \ref{output:pse} shows 
...
The IRQ affinity can be configured so that interrupts are handled on CPUs other than CPU 19. This helps reduce the number of kernel threads that are executed on CPU 19. 


The script \ref{script:irq_affinity} checks the smp\_affinity for each IRQ on a specific CPU and prints the IRQ numbers that are allowed to run on that CPU, sorted in ascending order. 

\vspace{1em}
\begin{minipage}{0.95\columnwidth}
	\begin{lstlisting}[name={Read IRQ affinity per CPU on system},label={script:irq_affinity}]
		#!/bin/bash
...
\vspace{1em}
\begin{minipage}{0.95\columnwidth}
\begin{lstlisting}[name={Output},label={output:irq_affinity}, breaklines=true]
...
%https://www.wikiwand.com/de/Advanced_Programmable_Interrupt_Controller
...
\begin{acronym}[XXXXX]
	...
	\acro{IRQ}[Interrupt Request]{Interrupt Request}
\end{acronym}
