...
\bigskip \noindent On top of that, a robotic application further confirmed the improvements. The difference between the command time and the time the signal reaches the PWM was measured 1,000 times for the untuned, tuned, and hardware versions of Salamander 4. The worst latency of the signal reaching the PWM dropped from an initial value of 129~ms to 3.988~ms. Comparing this to the worst latency value of the hardware version, which was 1.49~ms, it is apparent that the tuned virtualization through real-time performance configurations came very close to the determinism and reliability of the hardware.
