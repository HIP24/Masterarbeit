...
\bigskip \noindent \citeauthor{maPerformanceTuningKVMbased} \cite{maPerformanceTuningKVMbased} describe in their work how KVM impacts the real-time performance, which has helped this work gain insight into the base overhead of virtualization. 
When a physical interrupt occurs while the guest RTOS is running, there are at least six stages that need to be traversed before the interrupt can be delivered to the guest RTOS. Once the interrupt is delivered to the guest RTOS, it incurs additional latency since there is also the time between when an interrupt is recognized and the first instruction of the corresponding interrupt service routine (ISR) of the guest OS is started. These stages are the primary sources of latencies caused by KVM and depicted in Figure \ref{fig:six_stages_kvm}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\columnwidth]{img/six_stages_kvm.png}
	\caption[Six stages for the delivery of the interrupt to the guest]{Six stages for the delivery of the interrupt to the guest, followed by the guest IRT \cite{maPerformanceTuningKVMbased}}
	\label{fig:six_stages_kvm}
\end{figure}

Each of these stages is breifly described below:

\begin{enumerate}
    \item \textbf{Guest Code}: The guest OS is executing its normal operations.
    \item \textbf{Interrupt Happens}
    \item \textbf{VM Exit}: The host processes the VM exit, which involves handling the interrupt.
    \item \textbf{After VM Exit}: The host performs necessary operations after the VM exit.
    \item \textbf{Host ISR}: The host executes its ISR to handle the interrupt.
    \item \textbf{Host Scheduling}: The host schedules the guest VM to resume execution.
    \item \textbf{Before VM Entry}: The host prepares to re-enter the guest VM.
    \item \textbf{VM Entry}: The host completes the process of re-entering the guest VM.
    \item \textbf{Guest IRT}: The guest OS starts executing the ISR to handle the interrupt.
\end{enumerate}


\begin{comment}
and then apply real-time tunings on the host Linux and analyze 
how they can ease certain latencies incurred by harmful workloads.

here are two approaches to raising the priority of the guest RTOS process, specifically I/O thread and VCPU threads. On one hand, there is a Linux system call sched_ 
setscheduler() that can be invoked by KVM to set priority when creating those threads. 
On the other, Linux shell provides a command chrt that can be used by users to change 
priority after start-up.
\end{comment}
