...
\begin{table}[h!]
	...
	\centering
	\caption{Host report (Total of 445.908)}
...
\begin{itemize} 
	...
	\item \textbf{<idle>}: This represents the idle time of the CPU, hence it is not being used by any process. 
	\item \textbf{vhost}: A kernel module which improves virtio (virtual input/output) performance by handling virtqueues in the kernel, thereby reducing context switches and system calls. 
	...
	\item \textbf{migration/19}: The migration process is a kernel process that balances load across CPU cores by moving threads from one CPU to another. The number after the slash indicates the CPU core to which the migration process is bound. 
\end{itemize}
