...
\begin{table}[H]
	...
	{\renewcommand{\arraystretch}{1.2}% for the vertical padding
		\begin{tabular}{|l|l|l|l|l|}
			\hline
			\textbf{Samples} & \textbf{Lat Min (µs)} & \textbf{Lat Avg (µs)} & \textbf{Lat Max (µs)} & \textbf{Std Dev (µs)} \\ \hline
			1000             & 10.875                 & 15.072                  & 20.156                  & 2.921                   \\ \hline
		\end{tabular}}
\end{table}
...
\begin{table}[H]
	...
	{\renewcommand{\arraystretch}{1.2}% for the vertical padding
		\begin{tabular}{|l|l|l|l|l|l|}
			\hline
			\textbf{Tuning} & \textbf{Samples} & \textbf{Lat Min (µs)} & \textbf{Lat Avg (µs)} & \textbf{Lat Max (µs)} & \textbf{Std Dev (µs)} \\ \hline
			Hardware        & 1000             & 10.3                  & 8.5                   & 12.7                   & 0.9                   \\ \hline
			Untuned Virtualization    & 1000             & 12.654                & 40.671                & 394.126                & 52.711                \\ \hline
			Tuned Virtualization      & 1000             & 10.875                & 15.072                & 20.156                 & 2.921                 \\ \hline
		\end{tabular}}
\end{table}
...
\bigskip \noindent As a next step, the robotic application in a practical scenario also demonstrates and validates the improvement of the tuned virtualization compared to the unmodified virtualization. The difference between the command time and the response time of the robot was measured 1,000 times for the untuned, tuned and hardware version of Salamander 4. The most imortant metric here is also the worst response latency of the robot. It dropped from 394.126\textmu s to 20.156\textmu s in the developped application. Comparing this to the worst latency value of the hardware version, which is 12.7\textmu s, it is apparent that the tuned virtualization through real-time performance configurations came very close to the determinsim and reliability of the hardware. 
...
In this thesis, we successfully virtualized the Salamander 4 operating system and applied extensive real-time performance tunings to bring its latency closer to that of the bare metal setup. The results demonstrate that with the right configurations, it is possible to achieve deterministic behavior in a virtualized environment.

6.1 Summary
The initial latency gap between the virtualized and bare metal setups was significant.
Real-time performance tunings, including BIOS, kernel, host, and QEMU/KVM configurations, were applied.
The tuned virtualized setup showed a substantial reduction in latency and improved determinism.
6.2 Future Work
Future work could focus on further optimizing the virtualization layer and exploring other real-time performance tuning techniques. Additionally, testing the system under different workloads and scenarios would provide a more comprehensive understanding of its performance.

6.3 Practical Implications
The findings of this thesis have practical implications for industries relying on real-time systems. Virtualizing real-time operating systems can lead to cost savings and increased flexibility, provided that the necessary performance tunings are applied to ensure deterministic behavior.
\end{comment}
